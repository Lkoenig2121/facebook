...
export default function FriendsPage() {
  ...
  const fetchFriendRequests = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/friend-requests/${user.id}`);
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
      const data = await response.json();
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const filteredSuggestions = allUsers.filter((u: { id: string; name: string; avatar: string; location?: string }) => 
        u.id !== user.id && 
        !friendIds.has(u.id) &&
  ...
  const handleAcceptRequest = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/friend-requests/${requestId}/accept`, {
        method: 'POST',
      });
  ...
  const handleDeclineRequest = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/friend-requests/${requestId}/decline`, {
        method: 'POST',
      });
  ...
  const handleRemoveFriend = async (friendId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/friends/${user.id}/${friendId}`, {
        method: 'DELETE',
      });
        ...
        <div className="flex gap-6">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-xs text-gray-500">{friendRequests.length} requests</p>
                  </div>
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40">
            <div className="flex">
              <button
                onClick={() => setActiveTab('all')}
                className={`flex-1 py-3 text-center ${activeTab === 'all' ? 'text-blue-600 border-t-2 border-blue-600' : 'text-gray-500'}`}
              >
              ...
              <button
                onClick={() => setActiveTab('requests')}
                className={`flex-1 py-3 text-center relative ${activeTab === 'requests' ? 'text-blue-600 border-t-2 border-blue-600' : 'text-gray-500'}`}
              >
                <svg className="w-6 h-6 mx-auto mb-1" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M8 9a3 3 0 100-6 3 3 0 000 6zM8 11a6 6 0 016 6H2a6 6 0 016-6zM16 7a1 1 0 10-2 0v1h-1a1 1 0 100 2h1v1a1 1 0 102 0v-1h1a1 1 0 100-2h-1V7z" />
                </svg>
              ...
              <button
                onClick={() => setActiveTab('suggestions')}
                className={`flex-1 py-3 text-center ${activeTab === 'suggestions' ? 'text-blue-600 border-t-2 border-blue-600' : 'text-gray-500'}`}
              >
                <svg className="w-6 h-6 mx-auto mb-1" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M11 6a3 3 0 11-6 0 3 3 0 016 0zM14 17a6 6 0 00-12 0h12zM13 8a1 1 0 100 2h4a1 1 0 100-2h-4z" />
                </svg>
                  ...
                  <div className="space-y-4">
                    {friendRequests.map(request => (
                      <div key={request.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-xl">
                        <Link href={`/profile/${request.fromUserId}`} className="flex items-center space-x-4">
                          ...
                          <div>
                            <h3 className="font-semibold text-gray-800">{request.fromUserName}</h3>
                            <p className="text-sm text-gray-500">{getTimeAgo(request.timestamp)}</p>
                          </div>
                        </Link>
                        <div className="flex space-x-2">
                          ...
                          >
                            {processingRequests.has(request.id) ? 'Confirming...' : 'Confirm'}
                          </button>
                          <button 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {suggestions.map(suggestion => (
                      <div key={suggestion.id} className="border border-gray-200 rounded-xl overflow-hidden">
                        ...
                        <div className="p-4">
                          <Link href={`/profile/${suggestion.id}`}>
                            <h3 className="font-semibold text-gray-800 mb-1 hover:underline">{suggestion.name}</h3>
                          </Link>
                          {suggestion.location && (
                            <p className="text-sm text-gray-500 mb-3">{suggestion.location}</p>
                          )}
                          <button 
